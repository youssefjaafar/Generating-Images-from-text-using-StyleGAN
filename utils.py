...
def saveFeatureMaps(model, layer=1):
    ...
    if not os.path.exists(root):
        os.mkdir(root)
    for index1, m in enumerate(model.linear):
        print(m)
        if isinstance(m, nn.Linear):
            ...
            # img_array = img1*255
            # Image.fromarray(img_array.astype(np.uint8)).resize((400, 400)).save(current_dir + '/img1' + str(0) + '.png')
            ...
            #     img1 = np.expand_dims(np.asarray(f_map_img), 2).repeat(3,2)/255 * np.asarray(img_resized_img)
            #     Image.fromarray(img1.astype(np.uint8)).save(current_dir + '/feature_map_' + str(0) + '_' + str(f) + '.png')
...
def transform_img(img1, img_size):
    transforms = [
    ...
    return transform(img1).unsqueeze(0)


def compute_loss(f_maps, f_maps_ref, lambdas):
    assert len(f_maps) == len(f_maps_ref)

    len_f_maps = len(f_maps)
    loss = 0
    for i in range(len_f_maps):
        loss += (nn.functional.l1_loss(f_maps[i], f_maps_ref[i])/len_f_maps)*lambdas[i]
