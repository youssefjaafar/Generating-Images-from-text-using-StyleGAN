...
lr = arguments.lr
img_save_freq = arguments.img_save_freq
ref_img_path = arguments.ref_img_path
...
if ref_img_path is None:
    ...
    reference_image = clip_preprocess(Image.open(ref_img_path)).unsqueeze(0).to(device)
...
def compute_perceptual_loss(gen_image, reference_image):
    gen_image = torch.nn.functional.upsample_bilinear(img, (224, 224))
    loss = 0
    len_vgg_layer_mappings = int(max(vgg_layer_name_mapping.keys()))

    ref_feats = reference_image
    gen_feats = gen_image
...
while True:
    ...
    loss = compute_clip_loss(img, arguments.prompt)
    ...
    # perceptual_loss = compute_perceptual_loss(img, reference_image)
    ...
    if counter1 % arguments.img_save_freq == 0:
        img = tensor_to_pil_img(img)
        img.save(os.path.join(output_dir, f'{counter1}.png'))

        print(f'Step {counter1}')
